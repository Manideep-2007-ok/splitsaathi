        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="text-right shrink-0">
              <p className="text-sm font-semibold text-[var(--text-primary)] font-[JetBrains_Mono]">{formatCurrency(amount)}</p>
              {currentUserSplit > 0 && !isPaidByCurrentUser && (
                <p className="text-[10px] text-[var(--danger)] font-[JetBrains_Mono] mt-0.5">you owe {formatCurrency(currentUserSplit)}</p>
              )}
              {isPaidByCurrentUser && (
                <p className="text-[10px] text-[var(--success)] font-[JetBrains_Mono] mt-0.5">you lent {formatCurrency(amount - currentUserSplit)}</p>
              )}
            </div>
          </div>
          <div className="flex items-center gap-2 mt-3">
            ...
            {onDelete && (
              <button
                ...
                className="p-1 rounded-md text-[var(--text-muted)] hover:text-[var(--danger)] hover:bg-rose-500/10 transition-colors opacity-0 group-hover:opacity-100"
              >
