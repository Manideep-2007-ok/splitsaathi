...
function GroupExpensesLoader({ groupId, onExpensesLoaded }) {
  ...
  useEffect(() => { onExpensesLoaded(groupId, expenses); }, [groupId, expenses, onExpensesLoaded]);
  return null;
}
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start sm:items-center justify-between flex-col sm:flex-row gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-[var(--text-primary)] tracking-tight">Analytics</h1>
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none px-4 py-2.5 pr-10 text-sm rounded-xl bg-[var(--bg-surface)] border border-[var(--border-subtle)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:border-[var(--accent)] transition-all cursor-pointer"
          >
      ...
      {!groupsLoading && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: "Total Spent", value: formatCurrency(totalSpent), sub: `${allExpenses.length} expenses`, icon: TrendingUp, color: "from-[var(--accent)] to-[var(--accent-light)]" },
            { label: "Average", value: formatCurrency(averageExpense), sub: "per expense", icon: BarChart3, color: "from-emerald-400 to-teal-400" },
            { label: "Highest", value: formatCurrency(highestExpense), sub: "single expense", icon: PieChart, color: "from-amber-400 to-orange-400" },
          ].map((card) => (
            <div key={card.label} className="rounded-2xl p-5 glass-card relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-2xl font-bold text-[var(--text-primary)] font-money">{card.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl p-6 glass-card">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-base font-bold text-[var(--text-primary)]">Monthly Spending</h2>
        ...
        <div className="rounded-2xl p-6 glass-card">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-base font-bold text-[var(--text-primary)]">By Category</h2>
