...
import { Sparkles, User, Smartphone, ArrowRight, Loader2 } from "lucide-react";

function Onboarding() {
  const { currentUser, profileComplete, loading, markProfileComplete } =
  ...
  const upiError =
    trimmedUpi.length > 0 && !isValidUpiId(trimmedUpi)
    ...
    <div className="min-h-screen bg-[var(--bg-base)] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-lg">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-[var(--accent)] to-[#6366F1] shadow-2xl shadow-[var(--accent-glow)] mb-5 animate-float">
            <Sparkles className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-3xl font-extrabold text-[var(--text-primary)] font-[Syne] tracking-tight mb-2">
            Welcome aboard!
          </h1>
        ...
        <form
          ...
          className="bg-[#18181B]/80 backdrop-blur-xl border border-white/5 rounded-3xl p-8 shadow-2xl space-y-6"
        >
          ...
          <Input
            ...
            error={
              displayName.length > 0 && trimmedName.length === 0
                ? "Name is required"
                : undefined
            }
          />
          ...
          <Input
            ...
            helperText={
              !upiError
                ? "Used for receiving settlements. You can add this later."
                : undefined
            }
          />
